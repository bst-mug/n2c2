...
public class AdvancedCAD extends BaseClassifiable {
    ...
    static {
        ...
        // Matches also metoprolol
        DRUG_MARKERS.add(Pattern.compile("toprol", Pattern.CASE_INSENSITIVE));      // 258 times
        ...
        // TODO also spelled as "HYDROCLOROTH" (3 times)
        DRUG_MARKERS.add(Pattern.compile("hydrochlorothiazide", Pattern.CASE_INSENSITIVE)); // 100 times
        // TODO maybe match on *statin?
        DRUG_MARKERS.add(Pattern.compile("atorvastatin", Pattern.CASE_INSENSITIVE));    // 94 times
    ...
    static {
        // TODO test angin.*
        //ANGINA_MARKERS.add(Pattern.compile("anginal", Pattern.CASE_INSENSITIVE));     // 21 times
        // Leads to many fp due to negations
        //ANGINA_MARKERS.add(Pattern.compile("chest pain", Pattern.CASE_INSENSITIVE));
...
//        ANGINA_MARKERS.add(Pattern.compile(
//            "(?<!(no .{0,30}|bowel |limb |negative for |RLE |digital |enteric |extremity |denies .{0,50}))angina",
//            Pattern.CASE_INSENSITIVE));
//        ANGINA_MARKERS.add(Pattern.compile(
//            "(?<!(no.{1,150}|negative for.{1,150}|den.{1,150}|without.{1,30}|abscence of |any.{1,150}))chest pain",
//            Pattern.CASE_INSENSITIVE));
    ...
    static {
        ...
        // TODO maybe merge with ischemi.*
        ISCHEMIA_MARKERS.add(Pattern.compile(
            "(?<!(no .{0,30}|bowel |limb |negative for |RLE |digital |enteric |extremity |denie.{1,50}|without ))ischemia(?!( colitis))",
            Pattern.CASE_INSENSITIVE));

        // TODO test it
        //ISCHEMIA_MARKERS.add(Pattern.compile("dyspnea", Pattern.CASE_INSENSITIVE));
    }
